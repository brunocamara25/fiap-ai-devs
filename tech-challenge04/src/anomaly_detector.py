...
class AnomalyDetector:
    ...
    def __init__(self, history_size: int = 30,
        ...
        self.graphic_threshold = graphic_threshold
    ...
    def detect_anomalies(self, frame: np.ndarray, frame_number: int,
        ...
        # Inicializar tracking de anomalias se não existir
        if not hasattr(self, 'anomaly_tracker'):
            self.anomaly_tracker = {
                'last_detection': {},  # Último frame de detecção por tipo
                'active_anomalies': {},  # Anomalias ativas
                'cooldown_frames': 30   # Frames de cooldown entre detecções
            }
        ...
        # NOVO: Filtrar anomalias usando sistema de cooldown
        filtered_anomalies = []
        for anomaly in anomalies:
            atype = anomaly['type']
            
            # Verificar se está em cooldown
            last_frame = self.anomaly_tracker['last_detection'].get(atype, -100)
            if frame_number - last_frame < self.anomaly_tracker['cooldown_frames']:
                # Ainda em cooldown, apenas atualizar anomalia ativa
                if atype in self.anomaly_tracker['active_anomalies']:
                    self.anomaly_tracker['active_anomalies'][atype]['end_frame'] = frame_number
                    self.anomaly_tracker['active_anomalies'][atype]['duration'] += 1
                continue
            
            # Nova anomalia ou fim do cooldown
            if anomaly['confidence'] > 0.85:  # Apenas alta confiança
                # Registrar nova anomalia
                anomaly['start_frame'] = frame_number
                anomaly['end_frame'] = frame_number
                anomaly['duration'] = 1
                
                filtered_anomalies.append(anomaly)
                self.anomaly_tracker['last_detection'][atype] = frame_number
                self.anomaly_tracker['active_anomalies'][atype] = anomaly
        ...
        # Atualizar contadores apenas para anomalias filtradas
    ...
    def _detect_exaggerated_expressions(self, emotions: List[Dict], 
        ...
        for emotion in emotions:
            ...
            # Critério 2: Emoções específicas suspeitas
            suspicious_emotions = ['surprise', 'fear', 'disgust']
            if dominant in suspicious_emotions and confidence > 0.7:
                anomaly = {
                    'type': 'careta_exagerada',
                    'frame_number': frame_number,
                    'timestamp': frame_number / 30.0,
    ...
    def _detect_graphical_elements(self, frame: np.ndarray, 
        ...
        if saturation_ratio > 0.25:  # Mais de 5% dos pixels com saturação máxima
            # Validar que a área anômala é significativa
            total_anomalous_pixels = np.sum(high_saturation_mask)
            frame_pixels = frame.shape[0] * frame.shape[1]
            anomaly_percentage = total_anomalous_pixels / frame_pixels
            
            # Só considerar se for uma porção significativa do frame
            if anomaly_percentage < 0.10:  # Menos de 10% do frame
                pass  # Ignorar pequenas áreas
            else:
        ...
        if edge_density > 0.35:  # Muitas bordas detectadas
            anomaly = {
                'type': 'elemento_grafico',
                'frame_number': frame_number,
                'timestamp': frame_number / 30.0,
        ...
        if pure_colors['ratio'] > 0.20:  # Mais de 2% de cores puras
            anomaly = {
                'type': 'elemento_grafico',
                'frame_number': frame_number,
                'timestamp': frame_number / 30.0,
    ...
    def _detect_anomalous_patterns(self, frame: np.ndarray, 
        ...
        if freq_ratio > 0.5:  # Muita energia em alta frequência
            anomaly = {
